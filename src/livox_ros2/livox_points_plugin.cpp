...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
        ...
        // SDF には絶対パスを書けないため、相対パスのときは本パッケージの share から解決する。
        // 既存の自作プラグインと同じ規約。先頭が '/' なら絶対パスとしてそのまま使う。
        if (!file_name.empty() && file_name.front() != '/') {
            try {
                ...
                RCLCPP_ERROR(rclcpp::get_logger("LivoxPointsPlugin"),
                    "cannot resolve package share for csv path: %s", e.what());
            }
        }
        ...
        // PointCloud2 publisher。センサ点群は BEST_EFFORT が標準。下流（pointcloud_to_laserscan,
        // Autoware crop_box, GLIM 等）は SensorDataQoS で購読するため、RELIABLE だと QoS 不一致で
        // 受信できない。SensorDataQoS（BEST_EFFORT）で publish する。
    ...
    void LivoxPointsPlugin::OnNewLaserScans() {
        ...
        if (rayShape) {
            ...
            // 無効 ray（range が範囲外＝空振り）は (0,0,0) として残さずスキップする。
            // 元実装は range=0 で原点に大量の偽点を積み、下流のクラスタリング/SLAM が
            // 原点に巨大な点塊を見てしまっていた。有効点だけ詰め、最後に resize で切り詰める。
            int valid_count = 0;
