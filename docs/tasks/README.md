# タスク別ドキュメント

このディレクトリは、README のタスク一覧に対応する詳細ページを置く。各ページは「目的」「入出力」
「実行」「合格基準」「制約」を持つ。

| タスク | ページ |
|---|---|
| マッピング | [mapping.md](mapping.md) |
| ウェイポイント生成 | [waypoint_generation.md](waypoint_generation.md) |
| 巡回ナビ | [waypoint_navigation.md](waypoint_navigation.md) |
| 認識 | [recognition.md](recognition.md) |
| カラー点群出力 | [colorized_pointcloud.md](colorized_pointcloud.md) |

タスクの合格基準・制約を変える場合は、README や AGENTS.md ではなく該当ページを更新する。
