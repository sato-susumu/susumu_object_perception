...
| 出力（最終） | `outputs/waypoint_generation/<world>_waypoints.yaml`、`outputs/waypoint_generation/<world>_waypoints.png`、`outputs/waypoint_generation/<world>_waypoints_check.{json,md}`（契約名・git 追跡。waypoint YAML は `schema_version` と `provenance`、check JSON は `schema_version`、`validation_passed`、`summary`、詳細 `metrics`、入力 hash を持つ） |
...
- 検査ツール `scripts/check_waypoints.py`: 各点の clearance、placeable 自由空間のカバレッジ偏り、
  連続 edge の route clearance 上での到達性、最大測地ジャンプを数値判定する
  （壁に近い点 0、3m 超手薄 0、到達不能 edge 0、過大 jump 0）。
  `reached=N/N` だけでは見えない配置品質と route 品質を客観確認する。
...
ros2 run susumu_object_perception check_waypoints.py \
  --map outputs/mapping_indoor/indoor.yaml \
  --waypoints outputs/waypoint_generation/indoor_sparse_waypoints.yaml \
  --clearance 0.6 --connect-clearance 0.30 \
  --json-out outputs/waypoint_generation/indoor_sparse_waypoints_check.json \
  --md-out outputs/waypoint_generation/indoor_sparse_waypoints_check.md
```
...
### route edge 検査と relink 可視化 (iter、2026-06-26)

`check_waypoints.py` に route 品質検査を追加した。従来は clearance と coverage だけを見ていたため、
「各点は安全だが、連続 waypoint 間の geodesic edge が長い / route clearance 上で到達不能」
という合格基準の抜けを数値化できなかった。

追加検査:
- `--connect-clearance` / `--route-clearance`: waypoint が route passable 上の同一連結成分にあるかを見る。
- `--max-jump-warn`（既定 8m）: 連続 edge の A* 測地距離が過大かを見る。既存の live 合格版を
  デフォルトで NG にしないため既定は緩め。改善実験では 5m などに下げて使う。
- edge 長い順を出し、どの区間が問題かを座標付きで表示する。
- `--json-out` / `--md-out`: waypoint 数、最小 clearance、coverage max、route unreachable/over-jump
  数、長い edge 上位を summary として保存する。`--require-pass` を付けると NG 時に非ゼロ終了する。
  `run_all_tasks.sh` は indoor / break_room / cafe の各 waypoint 生成直後に
  `<world>_waypoints_check.{json,md}` を出す。iter23 以降の check JSON は `schema_version: 2` と短い
  `summary` を持ち、`validate_contracts.py` は indoor / break_room / cafe の
  `validation_passed=true` と `summary` の存在まで検査する。
  iter26 以降、`run_all_tasks.sh` の waypoint 生成フェーズはこの `--require-pass` が失敗したら
  ログだけで続行せず非ゼロ終了する。到達不能・壁近接・過大 edge がある点列を後段の巡回へ渡さない。
  iter30 以降、同フェーズは indoor / break_room / cafe の入力 map YAML/PGM が欠けている場合も
  スキップ扱いにせず非ゼロ終了する。全タスク末尾の `validate_contracts.py` も失敗時に `ls` で
  終了コードを上書きしない。
  iter48 以降の check JSON は `schema_version: 3` で、`inputs.map`、`inputs.map_image`、
  `inputs.waypoints` と、それぞれの SHA-256 (`inputs_hash`) を持つ。`validate_contracts.py` は
  現在の map YAML / map PGM / waypoint YAML と hash を照合し、地図や waypoint だけを差し替えて
  古い check を残す stale report を検出する。
  iter51 以降、`generate_waypoints.py` が出す waypoint YAML 本体も `schema_version: 2` と
  `provenance` を持つ。`provenance` には generator 名、入力 map YAML / PGM と SHA-256、
  生成パラメータ、hazard file の path/hash、採用 hazard 数を残す。`validate_contracts.py` は
  indoor / break_room / cafe の採用 waypoint YAML について、この provenance が現在の map YAML / PGM と
  一致することも検査する。屋外では段差/スタック event を `--hazard-file` で戻したとき、
  YAML 単体から「どの危険履歴を反映した点列か」を追跡できる。iter55 以降、
  `--hazard-file ~/...` のような home-relative path も provenance では `expanded_path` / `exists` /
  `sha256` を持つ。`/frontier_explore/hazards` の JSON (`hazards[{source,id,x,y,radius}]`) も
  そのまま入力でき、`source` は hazard 種別として保存される。

実験結果（採用成果物は上書きせず `experiments/waypoint_generation/2026-06-26_iter_route_check/`）:
- 既存 `indoor_waypoints.yaml` は厳しめ `--max-jump-warn 5.0` で 1 edge 警告
  （#4→#5、測地 6.51m）。既定 8m では OK、かつ live 完走実証済みなので採用版は維持。
- `--relink-long-jumps 3.0 --max-segment-length 3.0` の実験版は 15 WP、最大測地ジャンプ 2.98m、
  `--max-jump-warn 5.0` でも OK。ただし WP 数が増えるため、採用には live 巡回確認が必要。
- `generate_waypoints.py` の PNG は最終 `route_cells` を常に描くようにし、relink で同じ点を再訪する
  場合は番号を `2,6` のようにまとめて表示する。数値だけでなく画像でも route 順が確認できる。
...
- `visualization_msgs/Marker` (`LINE_STRIP`/`points`): <https://docs.ros2.org/foxy/api/visualization_msgs/msg/Marker.html>
...
- `scripts/visualize_patrol_result.py`: 巡回結果 JSON と地図から、各点を緑（reached）/黄（reached だが
  recovery 多発で苦戦）/赤（missed）で色分けした PNG を作る。`--json-out` / `--md-out` を指定すると
  reached/missed/struggled、recovery 合計、reason 別件数、duration 統計も保存する。巡回後 1 枚と
  summary で「どこで苦戦・スキップしたか」が分かる。
...
| 連続 edge が到達不能 / 長すぎる | `check_waypoints.py --route-clearance --max-jump-warn` | `--connect-clearance` / `--route-clearance` を見直す。必要なら `--relink-long-jumps` / `--max-segment-length` を実験し、採用前に live 巡回で確認 |
