...
4. **保存できる**
   `/slam/save_colorized_map` が `success: true` を返し、`outputs/colorized_pointcloud/` に PLY が保存される。
...
結果（成果物 `outputs/colorized_pointcloud/colorized_pointcloud_apriltag_calib.ply`、6412 点）— 合格基準 1〜5 を確認:
...
| 4. 保存できる | `/slam/save_colorized_map` が `success: true`、`outputs/colorized_pointcloud/` に PLY 保存 |
...
成果物: `outputs/colorized_pointcloud/colorized_pointcloud_{indoor,break_room}_apriltag_calib_final.ply` と
確認画像 `_check.png`。
