...
| `outdoor.wbt` | 屋外マッピング検証（**未対応**） | 20 m 平地 + 建物 2 棟 + 樹木 4 本。特徴が少なすぎて scan / SLAM の両立ができず、現状は未対応扱い（[`tasks/mapping_outdoor.md`](tasks/mapping_outdoor.md)） |
| `city_robot.wbt` | 車・歩行者・信号 + センサ付き TB3 | 認識・巡回・街環境の主対象。マッピング対象としては outdoor 同様 **未対応**扱い |
| `village_center.wbt` | 屋外マッピング実験用（**実用品質ではない**） | Webots 公式 `village_center.wbt` をパッケージ取り込み（SumoInterface 削除、TurtleBot3 を (-18.31, +97.52) の床パッチ上に追加）。フェンス・樹木・椅子・建物が密集。`webots_outdoor_mapping.launch.py explore_radius:=12.0` で半径制限して走らせる |
...
マッピングタスクの実用対象は屋内のみ（`indoor.wbt` / `break_room.wbt` / `cafe`）。屋外（`outdoor.wbt` /
`city_robot.wbt` / `village_center.wbt`）は実用品質に達していない。`kitchen.wbt` / `home.wbt` は存在しない。
