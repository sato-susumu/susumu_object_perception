...
> このドキュメントは**ハードウェア寄り** (Webots カメラ設定、 TF、 トピック構成、
> キャリブレーション手法) の解説。 全天球画像を使った**認識のしくみ** (透視ビュー
> 展開・8 分割・重なりの理由) は [`omni_camera_recognition.md`](omni_camera_recognition.md) を参照。
