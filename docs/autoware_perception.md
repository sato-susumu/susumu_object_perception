...
> `susumu_sim::PredictedCostmapLayer` が max 合成で焼く（下「Nav2 連携」）。検出・追跡そのものや
...
/velodyne_points (PointCloud2 / PointXYZI, frame: velodyne_link)
  ...
  │                                                                 │
  ├─[自作Py] shape_estimation_node.py              OBB 形状推定
  │     Autoware L字フィット踏襲 → /perception/detected_objects_shaped
  │
  ├─[自作Py] detection_by_tracker_node.py          過分割統合
  │     tracker 参照で統合(Cluster Merger) → /perception/detected_objects_merged
  │
  ...
  │
  ├─[自作Py] prediction_node.py                    将来軌跡予測
  │     2D 占有格子で CV 予測 + 壁回避
  │     → /perception/predicted_objects (PredictedObjects)
  │     → /perception/predicted_costmap (OccupancyGrid) ──→ Nav2 costmap の
  │         自作 predicted_layer(PredictedCostmapLayer) が max 合成（下「Nav2 連携」）
  │
...
- euclidean_cluster の DetectedObjects は位置のみで shape が空。各検出の中心から
  半径 `cluster_radius`(0.6m) 内の `/perception/no_ground/pointcloud` 点を集める。
- 角度 θ を 1°刻みで grid search（`optimize`）、各 θ で点を直交 2 軸に射影し
  closeness criterion（`calcClosenessCriterion`、d_min=0.1²/d_max=0.4² クランプ）を最大化。
- θ* から 4 辺の直線を立て交点で中心・寸法・yaw を出す（`fitLShape`）。
- 点が `min_points`(6) 未満なら人サイズの既定 BBox にフォールバック。
- 出力 `/perception/detected_objects_shaped` を map_roi_filter の入力にする。
...
**アルゴリズム**:
- tracker の各トラックを検出フレーム(velodyne_link)へ TF 変換し、位置と OBB 半径を得る。
- 各検出を最近傍トラック（中心間距離が `assign_radius`(0.4m) + トラック半径以内）に割り当て。
- 同一トラックに 2 個以上の検出が割り当たったら統合。**統合後の shape は包含 BBox では
...
- どのトラックにも属さない検出はそのまま通す（新規物体を消さない）。
...
- **CV 予測**: 各 tracked_object の現在速度で等速直線。`prediction_horizon`(3.0s) を
  `time_step`(0.5s) 刻みでサンプリングして `PredictedPath` の Pose 列にする
  （Autoware の既定挙動「現在速度で予測パス長を計算」と同じ）。
- **2D マップ壁回避**: 予測点を順にたどり、地図の occupied セル（>= `occupied_thresh`）に
  入ったらそこで打ち切る（壁にめり込む非現実的な予測を防ぐ）。地図外/未知は通す。
- **マルチモーダル**（Autoware の crosswalk マルチパスの 2D 版）: 進行方向を中心に
  複数角度 `angle_offsets_deg`（既定 -40/-20/0/+20/+40°）で扇状に複数の予測パスを出す。
  分岐路で「人がどっちに行くか」の複数候補を表現できる。各パスは壁で打ち切る。
- **confidence**: 直進(off=0)を最高に、外側ほど `cos(off)` で減衰。free で最後まで伸びた
  パスは加点、壁ですぐ切れたパスは減点し、全候補で正規化する。
- **静止物体**（速度 < `min_speed`）は予測パスを出さない（現在位置のみ）。
...
(frame=map)** として publish する。それを **自作 C++ costmap 層 `susumu_sim::PredictedCostmapLayer`**
...
- 静的 `/map` と同じ解像度・原点・サイズの格子を**毎フレーム全セル 0(free) から作り直す**
  （蓄積しない＝軌跡が残らない）。
- 人ごとに**ポリライン**を作る: 起点 = 現在位置（全トラック）、続き = 移動トラック（速度
  ≥ `min_speed`）の**最有力 1 本**の予測パスの近傍 `predcost_max_horizon`(2.0s) 以内。
  **confidence しきいは設けない**（移動トラックなら進路を必ず焼く＝出たり出なかったりを無くす）。
- ポリラインを map セルに変換し、**各点を `predcost_inflate_cells`(8=0.4m) 円盤膨張 + 連続点間を
  Bresenham 線分で繋いで**焼く（点を独立円で焼くと点間隔>円径で隙間＝飛び石になるため線分補間）。
  膨張は人幅 + tracker 推定方向と実進行方向のズレ吸収ぶん。マルチモーダル全 5 方向は焼かない
  （扇状に広がり costmap が埋まるため。可視化マーカーは全パスを出す）。

**なぜ標準 costmap 層でなく自作 C++ 層か（ライブ検証で判明した失敗の記録）:**
- **ObstacleLayer / STVL（点群方式）**: 予測パスを点群で焼くと、ObstacleLayer の raytrace
  clearing は「観測線上」しか消せず、人が向きを変えた**古い予測が蓄積して costmap が
  ぐちゃぐちゃ**（LETHAL 55%、ナビ不能）になった。STVL の極短 decay でも残像が残る。
- **StaticLayer（OccupancyGrid 方式）**: StaticLayer は地図を「真値」として扱い**後段の層が
  前段を上書き**するので、予測層（ほぼ全 free）が壁の static_layer を消して **壁が消失**
  （LETHAL 0%、costmap が真っ白でナビ不能）になった。
- **自作 `PredictedCostmapLayer`（採用）**: `updateCosts` で予測 OccupancyGrid の占有セルだけを
  **`max` で master costmap に乗せる**（他層を壊さない＝壁が消えない）。毎フレーム最新格子で
  置換するので**古い予測は蓄積しない**（前回の占有セルは今回 0 になり max でも乗らない）。
  「他層を壊さず・蓄積せず」を両立する。CMakeLists で C++ プラグインをビルドし pluginlib export。
