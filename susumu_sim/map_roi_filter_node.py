...
【入出力】
  in : /perception/detected_objects (DetectedObjects, frame=velodyne_link)
       /map (OccupancyGrid, latched)
  out: /perception/detected_objects_in_map (DetectedObjects, frame=velodyne_link)
...
class MapRoiFilterNode(Node):
    ...
    def on_detections(self, msg: DetectedObjects):
        ...
        out = DetectedObjects()
        out.header = msg.header  # frame は据え置き（velodyne_link）。下流は据え置きで動く
