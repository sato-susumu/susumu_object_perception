...
| C | Nav2 マップ + 3D LiDAR 障害物回避構成 | `nav2_params.yaml` / `outputs/mapping_*/` |
...
| ロボット | TurtleBot3 waffle + 追加 3D LiDAR（**MID-360 相当**、`lidar_model:=vlp16` で VLP-16 版も選択可）。**2D LiDAR は非搭載** |
| 3D LiDAR | Gazebo `ray` センサ + `liblivox_mid360_sensor.so`（LCAS/ODE MultiRayShape）で `/lidar/points`(PointCloud2, frame=`lidar_link`) を出力。VLP-16 版は `gpu_ray`+`gazebo_ros_ray_sensor` |
| 2D スキャン | `pointcloud_to_laserscan` が `/lidar/points` から `/scan`(frame=`lidar_link`) を生成（AMCL・obstacle_layer 用） |
...
├── CMakeLists.txt / package.xml   # ament_cmake。launch/config/urdf/worlds/outputs/rviz をinstall
...
| ファイル | 役割 |
|---|---|
| `models/turtlebot3_waffle_3d/model.sdf` | waffle SDF をコピーし、上部に **MID-360 相当 3D LiDAR** を追加。`liblivox_mid360_sensor.so`（LCAS/ODE MultiRayShape）が CSV の非反復角度列で `/lidar/points`(PointCloud2, frame=`lidar_link`) を出力。Gazebo へ spawn する実体。VLP-16 版は `models/turtlebot3_waffle_vlp16/model.sdf`（`gpu_ray`+`libgazebo_ros_ray_sensor.so`）。 |
| `urdf/turtlebot3_waffle_3d.urdf.xacro` | 標準 waffle URDF を include し、`base_link -> lidar_link` の TF フレームを追加。`robot_state_publisher` 用（TF専用、Gazeboプラグインは持たない）。 |

3D LiDAR 仕様（MID-360 相当）: 水平 360°、垂直 仰角 -7°〜+52°、range 0.1〜40 m、10 Hz、非反復スキャンパターン。詳細は [`docs/mid360_lidar_research.md`](docs/mid360_lidar_research.md)。
...
- `gz sdf -k model.sdf` → **Check complete**（rayセンサ + MID-360 プラグインが spec 適合）。
- `xacro turtlebot3_waffle_3d.urdf.xacro` → 展開成功、lidar_link を含む。
...
- HuNav wrapper 同梱の house マップ（`house.pgm` / `house.yaml`）を
  `susumu_object_perception/outputs/mapping_indoor/` にコピーして使用（SLAM 不要）。
...
>   （AMCL・obstacle_layer はこの生成 `/scan` を使う。frame は `lidar_link`）。
...
4. +12秒: RViz2（`rviz/simulation.rviz`、3D LiDAR PointCloud(`/lidar/points`)表示を追加済み）。
...
| 2D スキャン | `/scan` / frame `lidar_link`（**2D LiDAR は非搭載**。`pointcloud_to_laserscan` が `/lidar/points` から生成） | amcl scan_topic `scan`, obstacle_layer `/scan` | ✓ |
| 3D LiDAR | `/lidar/points` / frame `lidar_link` | Autoware perception 入力 + pointcloud_to_laserscan（→ /scan）。※ Nav2 costmap への 3D STVL 層は廃止 | ✓ |
...
```bash
ros2 topic echo /lidar/points --once           # 3D点群（PointCloud2、frame=lidar_link）
ros2 topic echo /people --once                 # HuNavの5人
ros2 topic list | grep -E "lidar|scan|people|human_states|cmd_vel"
ros2 run tf2_tools view_frames                 # TFツリー（map->odom->base_footprint->...->lidar_link）
```
...
cp ~/ros2_ws/src/hunav_gazebo_wrapper/maps/cafe.{pgm,yaml} ~/ros2_ws/src/susumu_object_perception/outputs/mapping_indoor/
