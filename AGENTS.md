...
> - **A 幾何的正しさ**: ①円形影なし（地面同心円が焼かれていない＝tiltAngle=0 前提）②壁が単一線
>   （同じ壁が二重・三重にブレない）③斜めノイズなし（自由空間を横切る誤マッチ線がない）
>   ④寸法が実環境と一致（例: indoor は床 5×10m）。
> - **B 完全性**: ⑤実環境の通行可能領域をほぼカバー（unknown の取り残しが少ない）⑥自由空間が
>   分断されず最大連結成分が free の大部分を占める（`scripts/map_progress_monitor.py` 等で確認）。
> - 対象 world は**実在する wbt のみ**: `indoor` / `break_room` / `outdoor` / `city_robot`。
>   `kitchen.wbt` / `home.wbt` は存在しない（過去 pgm は遺物）。屋外(outdoor/city)は特徴が乏しく
>   (20m 平地に建物/植木のみ) 地図が薄くなりやすい＝world 特性の限界として扱う。
