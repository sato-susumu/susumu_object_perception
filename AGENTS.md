...
| `…/fall_detector_node.py` | **転倒検知**。IMU(`/imu`) の roll/pitch から傾きを見てしきい値(45°)超えで転倒警告。odom は 2D 前提で roll/pitch=0 なので IMU を使う。`/fall_detector/status`(Bool)。webots_simulation/simulation の両 launch に統合（常時監視）。 quaternion → roll/pitch 計算は **iter93 で `geometry_utils.quat_to_roll_pitch` に集約** (math のみ軽量モジュール、 step_detector と共有) |
| `…/step_detector_node.py` | **段差検出 (iter16 で追加)**。 IMU 傾き 5-15° (転倒未満) + 加速度 z ジョルト (>4 m/s²) + cmd_vel-odom 進行率 (<30%) の 3 軸で段差/坂/スタックを検出。 `/step_detector/{status (Bool), event (JSON String), tilt_deg (Float32)}`。 屋外 launch (`webots_outdoor_mapping/waypoint_nav`) で起動。 fall_detector (45°) より低い閾値で「段差付近」 を捉え、 frontier_explore と waypoint_nav がイベントを購読して当該位置を blacklist 化する (= リカバリは別ノードに疎結合)。 全 3 イベント (tilt / stuck / accel_jolt) に **throttling** 適用 (iter29/39/49/56 整備): `confirm_sec=0.3` / `stuck_cooldown_sec=3.0` / `accel_jolt_cooldown_sec=2.0` で連発抑止。 quaternion → roll/pitch 計算は **iter93 で `geometry_utils.quat_to_roll_pitch` に集約** (fall_detector と共有) |
