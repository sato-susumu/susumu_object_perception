...
| タスク | 制約・合格基準の正本 |
|---|---|
| マッピング | [`docs/tasks/mapping.md`](docs/tasks/mapping.md) |
...
| ファイル | 役割 |
|---|---|
| `…/frontier_explore_node.py` | **frontier 探索による自律マッピング**。slam_toolbox が育てる `/map` のフロンティアを BFS でクラスタ化し、情報利得 `score=size_weight*log(size)-dist_weight*dist` で広い未踏領域を優先して NavigateToPose。`outdoor` / `city` では frontier 前に `perimeter` sweep で外周へ出る。到達不能ゴールはブラックリスト化、完了時 map_saver で自動保存。`launch/webots_city_mapping.launch.py` から起動 |
...
| `…/collision_diagnostic_node.py` | **衝突診断**。`/bumper/collision` の立ち上がりで scan/costmap/cmd_vel を突き合わせ、衝突原因を **A**(scan に障害物が乗らない=LiDAR 死角/高さ帯/range_min) / **B**(scan に有るが local_costmap に無い=obstacle_layer 設定/TF ドリフト) / **C**(costmap に有るのに前進指令=回避失敗) / **D**(scan/costmap 認識・前進指令なし=odom ドリフトや旋回接触) に切り分けてログ＋`/collision_diagnostic/event`(String, JSON) を出す。「移動物体の無い環境で衝突＝ナビの把握不全かアルゴリズム不良」を診断する用途。広域マッピングでは CPU を SLAM/Nav2 に回すため `webots_city_mapping.launch.py collision_diagnostics:=True` のときだけ起動 |

Webots マッピング/ナビの既知の罠（`mode:=realtime`、`tiltAngle 0`、FastRTPS SHM、検証プロセス掃除）は
[`docs/tasks/mapping.md`](docs/tasks/mapping.md)、[`docs/tasks/waypoint_navigation.md`](docs/tasks/waypoint_navigation.md)、
