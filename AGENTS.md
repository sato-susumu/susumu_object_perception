...
> **地図作成タスクのゴール条件（合格基準）**。ウェイポイント生成・巡回は**別タスク**で、
> 地図作成タスクは「地図そのものの品質」だけを対象にする。各 world について以下を満たせば合格:
> - **B【最重要】未開拓を残さず広く開拓する**: ⑤探索済み範囲(既知セルの bounding box)内に
>   unknown を残さず埋め尽くす ⑥到達可能な未踏領域を残さない（frontier を回り尽くす）
>   ⑦自由空間が分断されず最大連結成分が大半を占める。**これが地図作成の主目的**。`/scan` 点数は
>   広い環境では物理的に減る（全方位の大半が空に抜ける）ので気にしない。開拓が進んでいるかは
>   `scripts/map_progress_monitor.py`（区間移動量・開拓余地%＝bbox 内 unknown 率）で確認する。
>   移動量が十分あるのに地図が広がらないなら slam/環境、移動量が少ないなら探索アルゴリズムを疑う。
> - **A 幾何的正しさ**: ①円形影なし（tiltAngle=0 前提）②壁が単一線（二重・三重にブレない）
>   ③斜めノイズなし ④寸法が実環境と一致（例: indoor は床 5×10m）。
> - **C【最重要・必須】ワールド情報と地図のズレ確認**: 地図内部の整合（連結成分・壁率）だけ
>   では不十分。**wbt が定義する真の構造（Floor サイズ、Wall の translation/size、建物・木の
>   位置）と、出来上がった地図の壁(occ)・自由空間(free)の配置・寸法を照合**し、ズレが無いことを
>   必ず確認する:
>   - 地図の外周（部屋の壁）の寸法・位置が wbt の Wall 配置と一致するか（例: indoor は壁が
>     5×10m の矩形を囲む。地図がそれより小さい/大きい/歪んでいたら不合格）。
>   - wbt にある障害物（建物・木・家具）が地図の対応位置に occ として出ているか。
>   - 地図がロボット軌跡の放射状スキャンの寄せ集め（星形）で実際の壁構造を成していないなら不合格
>     （屋外で頻発。広く動いても壁を正しく結べていない）。
>   `scripts/eval_map_quality.py` の数値だけでなく、wbt の Wall translation/size を読んで地図に
>   重畳し**目視で照合**する（`scripts/check_map_vs_world.py` 参照）。
> - 対象 world は**実在する wbt のみ**: `indoor` / `break_room` / `outdoor` / `city_robot`。
>   `kitchen.wbt` / `home.wbt` は存在しない（過去 pgm は遺物）。屋外(outdoor/city)は特徴が乏しいが
>   (20m 平地に建物/植木のみ)、それでも frontier で到達可能範囲を広く開拓しきることを目指す。
> - 必ず `mode:=realtime`・全 wbt `tiltAngle 0` で作る（[Webots マッピングの罠](#) 参照）。
> - frontier 探索は未開拓優先（`size_weight` 大・`dist_weight` 小で遠方の大未踏へ積極展開）。
>
> ウェイポイント生成（`generate_waypoints.py`）と巡回（`waypoint_nav_node.py`）は地図が合格して
> から着手する別タスク。
...
| `…/collision_diagnostic_node.py` | **衝突診断**。`/bumper/collision` の立ち上がりで scan/costmap/cmd_vel を突き合わせ、衝突原因を **A**(scan に障害物が乗らない=LiDAR 死角/高さ帯/range_min) / **B**(scan に有るが local_costmap に無い=obstacle_layer 設定/TF ドリフト) / **C**(costmap に有るのに前進指令=回避失敗) / **D**(scan/costmap 認識・前進指令なし=odom ドリフトや旋回接触) に切り分けてログ＋`/collision_diagnostic/event`(String, JSON) を出す。「移動物体の無い環境で衝突＝ナビの把握不全かアルゴリズム不良」を診断する用途。`webots_city_mapping.launch.py` に統合（バンパー無し world では `/bumper/*` が来ないだけで無害） |
